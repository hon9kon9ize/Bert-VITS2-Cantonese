...
def run(rank, n_gpus, hps):
    ...
    if net_dur_disc is not None:
        ...
        optim_dur_disc = None
    net_g = DDP(
        net_g, device_ids=[rank], bucket_cap_mb=512, find_unused_parameters=True
    )
    net_d = DDP(net_d, device_ids=[rank], bucket_cap_mb=512)
    net_wd = DDP(net_wd, device_ids=[rank], bucket_cap_mb=512)
    if net_dur_disc is not None:
        net_dur_disc = DDP(
            net_dur_disc,
            device_ids=[rank],
...
def train_and_evaluate(
    ...
    for batch_idx, (
        ...
        x, attention_mask, x_lengths = (
            x.cuda(rank, non_blocking=True),
            attention_mask.cuda(rank, non_blocking=True),
